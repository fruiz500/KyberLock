...
function pasteLock() {
    setTimeout(function () {
        ...
        // Matches 6 slashes, captures exactly 1579 base64 chars, followed by 6 slashes
        var pbExchangeMatch = lock.match(/\/\/\/\/\/\/([A-Za-z0-9+\/]{1579})\/\/\/\/\/\//);
        if (pbExchangeMatch) {
            lock = pbExchangeMatch[1];
        }
        ...
        var lockLength = lockstripped.replace(/\s/g, '').length;
        if (lockLength == 3328 || lockLength == 1579) {
...
function addLock(fromMain) {
    ...
    // Smart extraction for Privacy Bar exchange format
    // Matches 6 slashes, captures exactly 1579 base64 chars, followed by 6 slashes
    var pbExchangeMatch = lock.replace(/<[^>]*>/g, '').match(/\/\/\/\/\/\/([A-Za-z0-9+\/]{1579})\/\/\/\/\/\//);
    if (pbExchangeMatch) {
        lock = pbExchangeMatch[1];
    }
    ...
    if (lock == '') {											//if box is empty, put a 50-character random base36 string there
        ...
        var locklength = stripTags(lock).replace(/<[^>]+>/g, '').replace(/[\s\r\n]/g, '').length;
        if (locklength == 3328 || locklength == 1579) {
            var lockcrypt = stripTags(lock).replace(/<[^>]+>/g, '').replace(/[\s\r\n]/g, '');							//store Locks unencrypted, everything else encrypted by the Key
...
function findLock() {
    ...
    if (index >= 0) {
        ...
        var lockLength = locDir[lockName][0].length;
        if (lockLength == 3328 || lockLength == 1579) {
            lockMsg.textContent = "Directory item found with name: " + lockName + "\r\nFingerprint:  " + lockPrint(locDir[lockName][0]);
...
function decryptLock() {
    ...
    if (lockBox.textContent.trim()) {
        ...
        if (lockBox.textContent.length > 500 && lockBox.textContent.length != 3328) {
...
function mergeLockDB() {
    ...
    if (lockstr.split('<br>').length > 1) {			//the real database merge implies multiline
        ...
        if (!refreshKey()) return;
        var seed32 = wiseHash(KeyStr, email, 32);
        KeySeed = nobleHashes.sha512(seed32);
...
function recryptDB(newKey, newUserName) {
    ...
    KeyDir = newKeyStretched;
    var email = readEmail();
    var seed32 = wiseHash(newKey, email, 32);
    KeySeed = nobleHashes.sha512(seed32);
...
function fillBox() {
    ...
    for (var i = 0; i < lockList.options.length; i++) {
        if (lockList.options[i].selected) {
            ...
            } else if (locDir[lockList.options[i].value][0].length > 500 && locDir[lockList.options[i].value][0].length != 3328 && locDir[lockList.options[i].value][0].length != 1579) {		//it's a Cover, so decrypt it and make it the new Cover
