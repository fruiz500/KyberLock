...
function secretshare(){
    ...
    var tags = main.match(/KL\d{2}p\d{3}/);                                 //in case of file
    if(!tags) tags = main.match(/part out of \d{3} split with KyberLock/);
    if(tags){																//main box has parts: join parts
        if(main.match('href="data:')){										//parts in links
            var shares = main.replace(/<div>/g,'<br>').replace(/<\div>/g,"").replace(/<b>/g,"").split("<br><br>").filter(Boolean)				//go from newline-containing string to array
        }else{
            var shares = mainBox.innerText.split("\n\n\n\n").filter(Boolean)							//split when double spaced
        }
        var	n = shares.length,
            quorum = parseInt(tags[0].match(/\d{3}/)[0]);       //quorum in tags is 3 digits in a row, first instance
        ...
        for (var i = 0; i < shares.length; i++) {
            ...
            var shareBin = decodeBase64(shares[i]);
...
function displayshare(shares,quorum){
    ...
    for (var i = 0; i < shares.length; i++) {
        var dataItem = encodeBase64(hex2charArray(shares[i].slice(1,length))).replace(/=+/g, '');
        if(i > 0) output += "<br><br>";
        if(fileMode.checked){
            output += '<a download="KL10p' + quorumStr + '.kyb" href="data:binary/octet-stream;base64,' + dataItem + '"><b>KyberLock Part out of ' + quorumStr + ' as a binary file</b></a>'
        }else{
            output += "<pre>----------begin part out of " + quorumStr + " split with KyberLock--------==\r\n\r\n" + dataItem.match(/.{1,80}/g).join("<br>") + "\r\n\r\n==--------end part out of " + quorumStr + " split with KyberLock----------</pre><br>"
        }
    };
