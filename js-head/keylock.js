...
function checkKey(key) {
    ...
    // Make seed from password and email
    KeySeed = wiseHash(key, myEmail, 64);
    // ML-KEM-768 for encryption
    myKemKeys = noblePostQuantum.ml_kem768.keygen(KeySeed);
    // ML-DSA-44 for signing (Level 2) - Shorter keys/signatures
    myDsaKeys = noblePostQuantum.ml_dsa44.keygen(KeySeed.slice(0, 32));

    if (!myLock) {
        // Total Lock length: 1184 (KEM) + 1312 (DSA) = 2496 bytes
        myLock = concatUi8([myKemKeys.publicKey, myDsaKeys.publicKey]);
        myLockStr = encodeBase64(myLock).replace(/=+$/, '');
    }
...
//creates fingerprint of a Lock in base36, for display. The lock is base64 of length 3136 or 1579
function lockPrint(lock) {
    if (lock.length == 3328 || lock.length == 1579) {
        var print = changeBase(lock.slice(0,12),base64,base36);
    } else {
...
function extractLock(string) {
    ...
    if (CGParts[0].length == 3328) {
        var possibleLock = CGParts[0];
        string = string.slice(4188)
    } else if (CGParts[1].length == 1579){
        var possibleLock = CGParts[1];          //from Privacy Bar
    } else {
        var possibleLock = removeHTMLtags(string)
    }

    if (possibleLock.length == 3328 || possibleLock.length == 1579) {
...
function lockElement() {
    ...
    el.textContent = "----------begin KyberLock 2.0 Lock--------==\r\n\r\n" + myLockStr.match(/.{1,80}/g).join("\r\n") + "\r\n\r\n==---------end KyberLock 2.0 Lock-----------";
    return el
}
...
function wiseHash(pwd, salt, length) {
    var logN = keyStrength(pwd, false);
    var N = Math.pow(2, logN); 
    
    // Direct sync call - no 'await' needed
    const output = nobleHashes.scrypt(
        new TextEncoder().encode(pwd), 
        new TextEncoder().encode(salt), 
        { N: N, r: 8, p: 1, dkLen: length }
    );
    
    return output; // Returns Uint8Array immediately
}
...
//    return nobleCiphers.xsalsa20poly1305(sharedKey, nonce).encrypt(plain)
    return nobleCiphers.secretbox(sharedKey, nonce).seal(plain)
}
...
function KLdecrypt(cipherStr, nonce, sharedKey, isCompressed, label) {
    ...
    try {
//        var plain = nobleCiphers.xsalsa20poly1305(sharedKey, nonce).decrypt(cipher);                         //it might not decrypt
        var plain = nobleCiphers.secretbox(sharedKey, nonce).open(cipher);                         //it might not decrypt
    }
    catch {
        failedDecrypt(label);
        return false
...
//        var cipher = nobleCiphers.xsalsa20poly1305(KeyDir, nonce).encrypt(plainstr)
        var cipher = nobleCiphers.secretbox(KeyDir, nonce).seal(plainstr)
    }
...
function keyDecrypt(cipherStr, isArray) {
    ...
    if (cipher[0] == 144) {
        ...
        if (isArray) {
            try {
//                var plain = nobleCiphers.xsalsa20poly1305(KeyDir, nonce).decrypt(cipher2);
                var plain = nobleCiphers.secretbox(KeyDir, nonce).open(cipher2);
                return plain
            }
            catch {
                return false
...
function replaceByItem(name) {
    ...
    if (index < 0) {									//not on database; strip it if it's a Lock
        ...
        if (stripped.length == 3328) { fullName = stripped } else { fullName = name }
    } else if (name == 'myself') {
        ...
        if (stripped.length == 3328) { fullName = stripped } else { fullName = whole }		//if it's a Lock, strip tags
    }
    return fullName
}
