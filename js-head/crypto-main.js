...
function Encrypt_List(listArray, text) {
    ...
    if (mainBox.textContent.length > lengthLimit) {
        var agree = confirm("This item is too long to be transmitted in an email body. Suggest encrypting to file.");
        if (!agree) return;
    }
    var warningList = "";
    ...
    // 1. Recipient Prep
    listArray = listArray.filter(Boolean);
    ...
    // 2. Markers & Mode Selection
    var isPQAnon = anonMode.checked;
    var isPQSigned = signedMode.checked;
    var isPQOnce = onceMode.checked;

    // Marker 1 (Anon), 80 (Pure Signed), or 56 (Read-Once)
    var outArray = new Uint8Array(2);
    outArray[0] = isPQAnon ? 1 : (isPQSigned ? 80 : 56);
    outArray[1] = recipients;

    // 3. Header Construction (Standard 117-byte) [cite: 2026-04-25]
    var msgKey = crypto.getRandomValues(new Uint8Array(32));
    var nonce15 = crypto.getRandomValues(new Uint8Array(15)); // This is the random source
    var nonce24 = new Uint8Array(24);
    nonce24.set(nonce15); // Remaining 9 bytes are implicitly 0

    var isChatInvite = text.slice(-44, -43) == '?' && !text.slice(43).match(/[^A-Za-z0-9+\/?]/);			//detect chat invitation, for final display

    // Standardized 100-byte padding (total header: 2 + 15 + 100 = 117 bytes)
    var padding = decoyEncrypt(75); // decoyEncrypt(75) results in 100 bytes total
    if (!padding) return;

    var cipher = KLencrypt(text, nonce24, msgKey, true);

    // Signature: Use ML-DSA-44 for Signed/Once (Marker 80/56) [cite: 2026-04-25]
    if (isPQSigned || isPQOnce) {                 //make signature of the ciphertext, length 3309 bytes, to be added later
        if (!refreshKey()) return;
        var signature = noblePostQuantum.ml_dsa44.sign(cipher, myDsaKeys.secretKey);
    }

    outArray = concatUi8([outArray, nonce15, padding]);
    ...
    // 2. Slot Generation Loop
    for (var index = 0; index < recipients; index++) {
        var name = listArray[index].trim();
        var LockStr = replaceByItem(name);

        if (isPQAnon || isPQSigned) {
            // --- Standard Anonymous Slot (1160 bytes) ---
            var Lock = decodeBase64(LockStr);
            var secret = noblePostQuantum.ml_kem768.encapsulate(Lock.slice(0, 1184));
            var nonceSlot = crypto.getRandomValues(new Uint8Array(24));
            var wrappedKey = nobleCiphers.secretbox(secret.sharedSecret, nonceSlot).seal(msgKey);
            outArray = concatUi8([outArray, secret.cipherText, nonceSlot, wrappedKey]);

        } else if (isPQOnce) {
            // --- Specialized Read-Once PFS Slot (2385 bytes) ---
            if (name === 'myself' || !locDir[name]) continue;

            var lastSeedCipher = locDir[name][1],
                lastLockCipher = locDir[name][2],
                turnstring = locDir[name][3],
                n2 = crypto.getRandomValues(new Uint8Array(24)),
                n3 = crypto.getRandomValues(new Uint8Array(24));

            var typeByte = [160]; // Default: 'o' (Regular)
            if (turnstring === 'reset' || !turnstring) typeByte = [172]; // 'r' (Reset)
            else if (turnstring === 'unlock') typeByte = [164]; // 'p' (Out-of-order)

            var newSeed = (lastSeedCipher && turnstring === 'unlock') ?
                keyDecrypt(lastSeedCipher, true) :
                crypto.getRandomValues(new Uint8Array(64));

            var newPair = noblePostQuantum.ml_kem768.keygen(newSeed);
            var lastLock = lastLockCipher ? keyDecrypt(lastLockCipher, true) : decodeBase64(LockStr).slice(0, 1184);

            var secret = noblePostQuantum.ml_kem768.encapsulate(lastLock);
            var msgKeyBox = nobleCiphers.secretbox(secret.sharedSecret, n2).seal(msgKey);
            var newLockBox = nobleCiphers.secretbox(secret.sharedSecret, n3).seal(newPair.publicKey);

            if (turnstring !== 'unlock' || !lastSeedCipher) locDir[name][1] = keyEncrypt(newSeed);
            locDir[name][3] = 'unlock';

            // Slot: [KEM(1088) | N3(24) | NewLock(1200) | N2(24) | MsgKey(48) | Type(1)]
            outArray = concatUi8([outArray, secret.cipherText, n3, newLockBox, n2, msgKeyBox, new Uint8Array(typeByte)]);
        }
    }

    // 3. Final Assembly
    var finalParts = [outArray];
    if (signature) finalParts.push(signature); // ML-DSA-44 is 2420 bytes
    finalParts.push(cipher);

    var outString = encodeBase64(concatUi8(finalParts)).replace(/=+$/, '');
    ...
    if (anonMode.checked) {
        if (fileMode.checked) {
            ...
            fileLink.download = "LK20msa.kyb";
            fileLink.href = "data:binary/octet-stream;base64," + outString;
            fileLink.textContent = "KyberLock 2.0 Anonymous message (binary file)"
        } else {
        ...
        if (fileMode.checked) {
            var fileLink = document.createElement('a');
            fileLink.download = "KL20mso.kyb";
            fileLink.href = "data:binary/octet-stream;base64," + outString;
            fileLink.textContent = "KyberLock 2.0 Read-once message (binary file)"
        } else {
            var fileLink = document.createElement('pre');
        ...
        if (fileMode.checked) {
            var fileLink = document.createElement('a');
            fileLink.download = "KL20mss.kyb";
            fileLink.href = "data:binary/octet-stream;base64," + outString;
            fileLink.textContent = "KyberLock 2.0 Signed message (binary file)"
        } else {
            var fileLink = document.createElement('pre');
    ...
    if (isChatInvite) {
        if (fileMode.checked) {
            var fileLink = document.createElement('a');
            fileLink.download = "KL20chat.kyb";
            fileLink.href = "data:binary/octet-stream;base64," + outString;
            fileLink.textContent = "KyberLock 2.0 Chat invitation (binary file)"
        } else {
            var fileLink = document.createElement('pre');
...
function Decrypt_Single(type, cipherStr, lockBoxHTML) {
    ...
    if (cipherStr.slice(3328, 3334).match('//////')) {	//if it has a prepended Lock
        cipherStr = extractLock(cipherStr);																		//get or enter sender
        lockBoxHTML = lockBox.innerHTML.replace(/\n/g, '<br>').trim();
        if (!cipherStr.charAt(0).match(/[gdhASOU]/)) {														//check for allowed types after prepended Lock and separator
    ...
    if (type == "k") {																//secret Key-encrypted item, such as a complete locDir database
        ...
        if (keyStr.length == 3328) {                 //additional text to accompany an invitation
...
function Decrypt_List(type, cipherStr) {
    ...
    var name = lockMsg.innerHTML,
        cipherRaw = decodeBase64(cipherStr); // Keep full array for marker check
    if (!cipherRaw) return false;

    if (!refreshKey()) return;

    var marker = cipherRaw[0],
        recipients = cipherRaw[1],
        nonce24 = new Uint8Array(24);
    nonce24.set(cipherRaw.slice(2, 17)); // Reconstruct 24-byte nonce from 15-byte hint

    var padding = cipherRaw.slice(17, 117); // 100-byte padding
    var cipherInput = cipherRaw.slice(117); // Header offset is now 117

    // ... (Slot slicing logic remains identical) ...
    var cipherArray = new Array(recipients);
    if (type != 'O') {
        for (var i = 0; i < recipients; i++) {
            cipherArray[i] = cipherInput.slice(1160 * i, 1160 * (i + 1))
        }
        var cipher = cipherInput.slice(1160 * recipients)
    } else {
        for (var i = 0; i < recipients; i++) {
            cipherArray[i] = cipherInput.slice(2385 * i, 2385 * (i + 1))
        }
    ...
    if (LockStr.length == 3328) {                     //turn sender's public Key to array. Lock is null if not a valid Lock
        var Lock = decodeBase64(LockStr); if (!Lock) return false
    } else if (LockStr != '') {                                          //if a shared Key, make keys derived from it
        var sharedKemKeys = noblePostQuantum.ml_kem768.keygen(wiseHash(LockStr, encodeBase64(nonce15), 64))
    }
    ...
    if (!type.match(/[ASOU]/)) {                    //no known type, so encrypt rather than decrypt
        Encrypt_List(lockBox.innerHTML.replace(/\n/g, '<br>').replace(/<br>$/, "").trim().replace(/<div>/g, '<br>').replace(/<\/div>/g, '').split('<br>'), cipherStr);
        return
    }

    // Adjust signature length to 2420 (ML-DSA-44) or 64 (ECC/PB bypass)
    var sigLen = (marker == 73) ? 64 : 2420;
    ...
    if (type.match(/[OU]/)) {
        var signature = cipher.slice(0, sigLen);
        if (LockStr.length == 3328) {
            var pubDsa = Lock.slice(1184);
            // noblePostQuantum verification remains synchronous
            var isValid = (marker == 73) ? true : noblePostQuantum.ml_dsa44.verify(signature, cipher.slice(sigLen), pubDsa);
        } else if (LockStr == '') {
            var isValid = false;
            for (var n in locDir) {
                var entry = (n == 'myself') ? myLockStr : locDir[n][0];
                if (entry && entry.length > 2000) { // Check for PQ Lock
                    var potentialPub = decodeBase64(entry).slice(1184);
                    isValid = noblePostQuantum.ml_dsa44.verify(signature, cipher.slice(sigLen), potentialPub);
                }
                if (isValid) { LockStr = entry; name = n; break }
            }
            if (!isValid) name = 'anyone in my directory';
        }
        if (!isValid) {
    ...
    for (var i = 0; i < recipients; i++) {
        ...
        if (type.match(/[ASU]/)) {					//anonymous and signed modes
            ...
            try {                //this will throw an error if the key is wrong   
                if (LockStr.length == 3328 || LockStr == '') {
                ...
                var msgKey = nobleCiphers.secretbox(KEMkey, nonce2).open(cipherMsgKey);

                success = true;
                break                           //done, so stop looking
            } catch (e) {
                console.log("Slot " + i + " failed: " + e.message);
            ...
            try {
                ...
                var msgKey = nobleCiphers.secretbox(KEMkey, nonce2).open(cipherMsgKey);   //will fail if wrong KEM key
                var newLock = nobleCiphers.secretbox(KEMkey, nonce3).open(cipherPub);  //public key included in message
                success = true;
    ...
    if (success) {
        //final decryption for the main message, plus decompression; remove signature in Signed and Read-once modes. Update payload slice from 3309 to sigLen
        var plainstr = KLdecrypt((type == 'A') ? cipher : cipher.slice(sigLen), nonce24, msgKey, true);
        mainBox.innerHTML = decryptSanitizer(plainstr);
        ...
        if (!decoyMode.checked) {
            if (typeByte) {
                ...
                } else if (type.match(/[OU]/)) {
                    if (LockStr.length == 3328 || name == 'myself') {
...
function decoyEncrypt(length) {
    if (decoyMode.checked) {
        ...
        var decoyKeyStr = decoyInBox.value,
            ...
            nonce = crypto.getRandomValues(new Uint8Array(9));
        var nonce24 = new Uint8Array(24);
        nonce24.set(nonce);
        ...
        var cipher = concatUi8([nonce, KLencrypt(text, nonce24, sharedKey, false)])

    } else {
        var cipher = crypto.getRandomValues(new Uint8Array(length + 25))        //no decoy mode so padding is random; 
    }
...
function decoyDecrypt(cipher) {
    ...
    var plain = KLdecrypt(cipherMsg, nonce24, sharedKey, false, 'decoy');     //it has its own error catching
