...
encodePNGBtn.addEventListener('click', handleEncodePNG);

encodeJPGBtn.addEventListener('click', handleEncodeJPG);

decodeImgBtn.addEventListener('click', handleDecodeImage);
...
// Attach the search listener after the UI is ready
const searchInput = document.getElementById('helpSearch');
if (searchInput) {
    searchInput.addEventListener('keyup', function() {
        const query = this.value.toLowerCase();
        const details = document.querySelectorAll('#helpTab details');
        const headings = document.querySelectorAll('#helpTab .helpHeading');

        details.forEach(detail => {
            const text = detail.innerText.toLowerCase();
            if (text.includes(query)) {
                detail.style.display = "block"; // Note: .style property is allowed in JS, just not HTML
                detail.open = (query.length >= 3);
            } else {
        ...
        headings.forEach(heading => {
            ...
            if (container && container.classList.contains('helpText')) {
                const visibleDetails = container.querySelectorAll('details[style="display: block;"]');
                heading.style.display = (query.length > 0 && visibleDetails.length === 0) ? "none" : "block";
            }
        });
    });
}
...
if (!localStorage || localStorage.length == 0) {
    ...
    // If you want to be more specific than just .length, 
    // you could check for a key you know your app always creates.
    fillNameList();
}
